...
- [ ] **Nothing accounted for by silence** — every persona gap named in a persona card either drives a rec (it's in the Source line of at least one recommendation) or is explicitly named in "What we set aside" with a reason. A gap visible in a persona card that appears nowhere else is the worst kind of omission — the builder sees it and can't find the resolution. Same rule for all 5 lenses and every category-adjacent domain.
...
- [ ] **Ratings actually sort** — not more than half the recs in the top ★ tier, AND not more than half at the same effort (S/M/L). If both axes collapse, the ranking is asserted, not computed — re-spread or state the one-clause tiebreak when two recs share both ★ and effort. **Specific trap:** if rec #N+1 has the same ★ but lower effort than rec #N, the formula ("impact ÷ effort") says #N+1 should rank above #N. If you're inverting this order, state explicitly why (e.g. a sequencing dependency — "#N must ship first") so the builder knows the inversion is intentional, not a mistake.
