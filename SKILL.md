...
- [ ] **Sequencing is consistent** — if rec #X says "after #N," then rec #N's Sequencing cannot say "none." Scan all sequencing fields as a pair-check before outputting.
...
- [ ] **Competitor claims are tagged** — any named competitor, named product, or "no competitor does X" assertion that you did not directly verify gets `[unverified]` inline. This applies to claims in Elevation items especially. The builder needs to know what's checked vs. inferred.
