...
**Selecting the least-technical persona:** Ask whether a non-developer who uses Claude through a browser (not Claude Code) could plausibly benefit from this skill's *domain* — not just the skill itself. If yes (e.g. health, finance, productivity), include them — but only if their gap drives a real recommendation. If the only gap such a person has is "can't install skills directly" (unsolvable within the skill, especially under a distribution goal), don't card them — every persona must drive at least one ranked recommendation. Select instead the lowest-technical person who *does* have a solvable gap: an internal team member who has Claude Code but avoids skill configuration, or a new adopter who hasn't built skill literacy yet. Same when the skill's domain is inherently technical (code review, CLI tools). This selection is internal reasoning — the output presents whichever persona you chose as itself, with no mention of who was considered and set aside.
